...
from gluon_retinace.utils import RetinaFace
...
class RetinaFaceDetect(object):
    def __init__(self, gpu=0):
        ...
        self._model = self.load_model()
        self._utils = RetinaFace(self._gpu, 'net3')
    ...
    def load_model(self):
        ...
        json_path = file_path + sep + '../model/test-symbol.json'
        param_path = file_path + sep + '../model/test-0000.params'
        #detector = RetinaFace('./model/test', 0, self._gpu, 'net3')
        #
        # net = nn.SymbolBlock.imports(json_path, ['data'], param_file=param_path,
        #                              ctx=mx.gpu())
        gluon_net = RetinfaceGluon()
        gluon_net.load_parameters('./gluon_retinace/gluon_net.params',ctx=mx.gpu(0))
        return gluon_net
    ...
    def detect(self, img, is_resize=True, target_size=512):
        ...
        im_tensor_mx = mx.nd.array(im_tensor, ctx=mx.gpu(0))
        output = self._model(im_tensor_mx)
        faces, landmarks = self._utils.detect(im_tensor,output, self._thresh, im_scale)
...
if __name__ == '__main__':
    ...
    for img in imgs:
        name = img.split('/')[-1]
        img = cv2.imread(img)
        results = detector.detect(img, is_resize=False, target_size=512)
